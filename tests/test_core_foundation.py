    ...
    def test_manual_catalog_sync_imports_updates_and_keeps_items_unpublished(self) -> None:
        ...
        def fake_urlopen(request, timeout=0):
            ...
            if "/entity/assortment" in request.full_url:
                ...
                return FakeMoyskladResponse(json.dumps(payload).encode("utf-8"))
            if "/report/stock/all" in request.full_url:
                payload = {
                    "rows": [
                        {
                            "stock": 7,
                            "reserve": 1,
                            "inTransit": 0,
                            "assortment": {"meta": {"href": "https://api.moysklad.ru/api/remap/1.2/entity/product/sku-1"}},
                        },
                        {
                            "stock": 0,
                            "reserve": 0,
                            "inTransit": 0,
                            "assortment": {"meta": {"href": "https://api.moysklad.ru/api/remap/1.2/entity/product/sku-zero"}},
                        }
                    ]
                }
                return FakeMoyskladResponse(json.dumps(payload).encode("utf-8"))
        ...
        try:
            first = run_manual_catalog_sync(app.conn, user["id"])
            second = run_manual_catalog_sync(app.conn, user["id"])
        finally:
            urllib.request.urlopen = original_urlopen

        self.assertEqual(first["stats"]["folderMatched"], 2)
        self.assertEqual(first["stats"]["found"], 1)
        self.assertEqual(first["stats"]["skippedNoPositiveAvailability"], 1)
        ...
        self.assertEqual(products[0]["stock_quantity"], 7)
        ...
        self.assertEqual(app.conn.execute("SELECT COUNT(*) FROM moysklad_sync_logs").fetchone()[0], 2)
    ...
    def test_manual_catalog_sync_recursively_includes_child_product_folders(self) -> None:
        ...
        def fake_urlopen(request, timeout=0):
            ...
            if "/entity/assortment" in request.full_url:
                ...
                return FakeMoyskladResponse(json.dumps(payload).encode("utf-8"))
            if "/report/stock/all" in request.full_url:
                return FakeMoyskladResponse(json.dumps({"rows": [{"stock": 2, "assortment": {"meta": {"href": sku_href}}}]}).encode("utf-8"))
            raise AssertionError(request.full_url)
