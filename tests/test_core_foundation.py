...
class CoreFoundationTest(unittest.TestCase):
    ...
    def test_moysklad_container_inference_recognizes_common_can_markers(self) -> None:
    ...
    def test_public_content_settings_drive_home_and_navigation(self) -> None:
        ...
        self.assertIn("--mobile-fixed-bg:linear-gradient(180deg, rgba(16,88,89,.84)", html)
    ...
    def test_manual_container_override_drives_catalog_and_survives_product_sync_updates(self) -> None:
        app = self.make_app()
        ...
        self.assertEqual(admin_catalog_items(app.conn)[0]["container_type_override"], "can")
        ...
        # MoySklad synchronization updates the source classification, not the explicit admin override.
        app.conn.execute("UPDATE products SET container_type = 'keg' WHERE id = ?", (product_id,))
        app.conn.commit()
        self.assertEqual(public_catalog(app.conn)["items"][0]["containerType"], "can")
