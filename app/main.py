        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                if path == PUBLIC_ORDER_API_ROUTE:
                    ...
                    customer_email = str(customer["email"] if customer is not None else "guest@stammbrewing.local")
                    company_name = str(customer["counterparty_name"] if customer is not None else "Гость B2B")
                    inn = str(customer["inn"] if customer is not None else "")
                    source = {
                    ...
                    cursor = app.conn.execute(
                        ...
                        INSERT INTO b2b_orders (
                            number, contact_name, company_name, inn, email, phone, city, total_minor, currency, source_json
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'RUB', ?)
                        """,
                        (order_number, customer_email, company_name, inn, customer_email, "—", "—", total_minor, json.dumps(source, ensure_ascii=False)),
                    )
                    ...
                    app.conn.commit()
                    self.send_json({"ok": True, "orderNumber": order_number, "totalMinor": total_minor})
                    return
                if path == "/account/register":
                    form = self.read_form()
