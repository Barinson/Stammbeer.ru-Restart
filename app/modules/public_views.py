...
from app.modules.content.service import ACTION_DEFAULTS, BUSINESS_DEFAULTS, CONTACT_DEFAULTS, HOME_DEFAULTS, MENU_DEFAULTS
...
def contacts_page(content: dict[str, Any] | None = None) -> str:
    ...
    email_cards = "".join(
        ...
        for item in emails if item.get("value")
    ...
    phone_cards = "".join(
        ...
        for item in phones if item.get("value")
...
{BASE_CSS}
    ...
    .contacts-card h1 {{ margin:0 0 12px; color:var(--golden-malt); text-transform:uppercase; letter-spacing:.08em; font-size:42px; }}
    .contacts-card p {{ color:rgba(246,241,227,.78); line-height:1.55; }}
    ...
    .contact-list span {{ color:rgba(246,241,227,.58); font-size:13px; text-transform:uppercase; letter-spacing:.08em; }}
    .contact-list a, .contact-list strong {{ color:var(--foam); text-decoration:none; font-size:18px; }}
    .map-card {{ overflow:hidden; min-height:420px; padding:0; display:grid; }}
    .map-card iframe {{ width:100%; min-height:420px; border:0; filter:saturate(.92); }}
    .map-caption {{ padding:18px 22px; background:rgba(11,63,64,.94); border-top:1px solid rgba(199,177,102,.18); }}
    .map-caption strong {{ color:var(--golden-malt); }}
    .map-caption p {{ margin:6px 0 0; color:rgba(246,241,227,.72); }}
  ...
  <main class="contacts-page">
    <section class="contacts-hero">
      <div class="contacts-card">
        ...
        <ul class="contact-list"><li><span>Адрес завода</span><strong>{escape(address)}</strong></li></ul>
      </div>
      <div class="contacts-card map-card">
        <iframe title="Яндекс.Карта: {escape(title)}" src="{map_src}" loading="lazy" allowfullscreen></iframe>
        <div class="map-caption"><strong>{escape(title)}</strong><p>{escape(address)}</p></div>
      </div>
    </section>
  </main>
{age_gate_markup()}
...
{BASE_CSS}
    ...
    h2 {{ margin:0; font-size:16px; line-height:1.15; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--white); font-weight:600; }}
    ...
    .cart__comment label {{ display:grid; gap:6px; color:rgba(246,241,227,.72); font-size:12px; font-weight:600; }}
