...
def _discount_debug_html(customer: Any) -> str:
    ...
    lines = [
        ("counterparty id", payload.get("counterpartyId") or diagnostics.get("counterpartyId")),
        ("name", payload.get("counterpartyName") or diagnostics.get("counterpartyName")),
        ("inn", payload.get("counterpartyInn") or diagnostics.get("counterpartyInn")),
...
{public_nav("business", content)}
  ...
  <script>
    ...
    function updateDiagnostics(meta) {{
      ...
      diagnosticsEl.textContent = `Источник: ${{meta.readModel}} · товаров: ${{meta.returnedItems}}/${{meta.totalLocalItems}} · обновлено: ${{updated}}${{discount}}`;
    }}
