...
from app.modules.content.service import ACTION_DEFAULTS, BUSINESS_DEFAULTS, HOME_DEFAULTS, MENU_DEFAULTS
...
BASE_CSS = """
    ...
    * { box-sizing:border-box; }
    ...
    .nav-links { display:flex; align-items:center; justify-content:center; gap:clamp(34px,4vw,64px); flex-wrap:wrap; font-size:14px; font-weight:700; }
...
ACCOUNT_CSS = """
    ...
    .account-card h1 { margin:0 0 10px; color:var(--golden-malt); font-size:clamp(34px,6vw,60px); line-height:.95; letter-spacing:.08em; text-transform:uppercase; }
    .account-card p { color:rgba(246,241,227,.78); font-size:17px; line-height:1.5; }
    .account-form { display:grid; gap:14px; margin-top:24px; }
    .account-form label { display:grid; gap:6px; color:rgba(246,241,227,.8); font-weight:700; }
...
{BASE_CSS}
    ...
    .price-debug {{ margin-bottom:14px; border-radius:14px; padding:10px 12px; background:rgba(11,63,64,.42); color:rgba(246,241,227,.72); font-size:12px; white-space:pre-wrap; }}
    ...
    .price-source {{ color:rgba(246,241,227,.6); font-size:11px; font-weight:800; }}
    ...
    .cart__comment label {{ display:grid; gap:6px; color:rgba(246,241,227,.72); font-size:12px; font-weight:800; }}
    ...
    .cart__minimum {{ margin-top:10px; color:rgba(246,241,227,.72); font-size:12px; line-height:1.35; }}
...
{public_nav("business", content)}
  <main class="wrap">
    ...
    <div class="price-debug" id="priceDebug" hidden></div>
      ...
      <aside class="cart" id="cart" aria-live="polite">
        ...
        <div class="cart__comment">
          <label>Комментарий к заказу
            <textarea id="orderComment" maxlength="1000" placeholder="Комментарий для менеджера и МойСклад"></textarea>
          </label>
        </div>
  ...
  <script>
    ...
    const priceDebugEl = document.getElementById('priceDebug');
    ...
    function setState(title, text, branch = 'loading') {{
      ...
      diagnosticsEl.dataset.uiBranch = branch;
    ...
    function renderCards(items) {{
      ...
      gridEl.innerHTML = items.map((item) => {{
        ...
        const basePrice = item.price.isPersonalized ? `<span class="price-base">${{escapeHtml(item.price.baseLabel)}}</span>` : '';
        const priceSource = item.price.pricingSource === 'price_type' && item.price.priceTypeName
          ? `<span class="price-source">${{escapeHtml(item.price.priceTypeName)}}</span>`
          : '';
        ...
        <article class="product">
          ...
          <div class="product__body">
            <div class="badges"><span class="badge">${{safeContainer}}</span>${{stepHint}}</div>
            <h2>${{safeName}}</h2>
            <div class="meta"><span class="price">${{safePrice}}</span>${{basePrice}}${{priceSource}}</div>
          </div>
      ...
      stateEl.hidden = true;
      gridEl.hidden = false;
      diagnosticsEl.dataset.uiBranch = 'success';
    }}

    function updateDiagnostics(meta) {{
      const updated = meta.lastCatalogSyncAt || 'ещё не обновлялся';
      const discount = meta.customerDiscountPercent ? ` · персональная скидка: ${{meta.customerDiscountPercent}}%` : '';
      const priceType = meta.customerPriceType?.name ? ` · тип цен: ${{meta.customerPriceType.name}}` : '';
      diagnosticsEl.textContent = `Источник: ${{meta.readModel}} · товаров: ${{meta.returnedItems}}/${{meta.totalLocalItems}} · обновлено: ${{updated}}${{discount}}${{priceType}}`;
    }}

    function renderPriceDebug(meta) {{
      if (!priceDebugEl) return;
      const sample = meta.priceDebugSample || [];
      if (!sample.length) {{
        priceDebugEl.hidden = true;
        return;
      }}
      const customerType = meta.customerPriceType?.name || 'гость / базовая цена';
      const rows = sample.slice(0, 5).map((entry) => {{
        const types = (entry.availablePriceTypes || []).map((price) => `${{price.priceTypeName || '—'}}=${{formatMoney(price.value)}}`).join(', ') || 'нет';
        const matched = entry.matchedPriceType ? `${{entry.matchedPriceType.priceTypeName}}=${{formatMoney(entry.matchedPriceType.value)}}` : 'нет совпадения';
        return `${{entry.skuName}} | base=${{formatMoney(entry.basePriceMinor)}} | types=${{types}} | matched=${{matched}} | returned=${{formatMoney(entry.returnedAmountMinor)}} | source=${{entry.pricingSource}}`;
      }});
      priceDebugEl.textContent = `Price debug · customer priceType: ${{customerType}}\n${{rows.join('\\n')}}`;
      priceDebugEl.hidden = false;
    }}
    ...
    function renderCart() {{
      ...
      if (entries.length === 0) {{
        cartBodyEl.innerHTML = `<div class="cart__empty" aria-label="Корзина пуста"></div><div class="cart__minimum">Минимальная сумма заказа: ${{escapeHtml(minimumOrderLabel)}}.</div>`;
        return;
      }}
      const rows = entries.map(({{ item, quantity }}) => {{
        const lineTotal = (item.price.amountMinor || 0) * quantity;
        const cartPriceSource = item.price.pricingSource === 'price_type' && item.price.priceTypeName ? ` · ${{escapeHtml(item.price.priceTypeName)}}` : '';
        return `
          <div class="cart-item">
            <div>
              <div class="cart-item__name">${{escapeHtml(item.name)}}</div>
              <div class="cart-item__meta">${{escapeHtml(item.containerLabel)}} · ${{escapeHtml(item.price.label)}}${{cartPriceSource}} × ${{escapeHtml(quantity)}}</div>
            </div>
      ...
      cartBodyEl.innerHTML = `
        ...
        <div class="cart__minimum">Минимальная сумма заказа: ${{escapeHtml(minimumOrderLabel)}}.</div>
        ${{isBelowMinimum ? `<div class="cart__message is-error">До оформления осталось ${{escapeHtml(formatMoney(minimumOrderAmountMinor - totalMinor))}}. Увеличьте заказ до минимальной суммы.</div>` : '<div class="cart__message">Минимальная сумма набрана, заявку можно оформить.</div>'}}
    ...
    async function loadCatalog() {{
      ...
      try {{
        ...
        diagnosticsEl.dataset.apiStatus = String(response.status);
        ...
        diagnosticsEl.dataset.itemsLength = String(items.length);
        updateDiagnostics(meta);
        renderPriceDebug(meta);
        ...
        diagnosticsEl.dataset.errorMessage = message;
        diagnosticsEl.textContent = `Источник: локальный каталог · ошибка API · ${{message}}`;
