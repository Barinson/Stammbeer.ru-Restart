...
BASE_CSS = """
    ...
    * { box-sizing:border-box; }
    ...
    .nav-icon { width:32px; height:32px; border:1px solid rgba(199,177,102,.28); border-radius:999px; display:grid; place-items:center; color:var(--foam); text-decoration:none; font-size:13px; font-weight:900; overflow:hidden; }
    .nav-icon img { width:18px; height:18px; object-fit:contain; display:block; }
...
def public_nav(active: str, content: dict[str, Any] | None = None) -> str:
    ...
    for item in site_content["actions"]:
        ...
        action_links.append(
            f'<a class="nav-icon {"nav-icon--cart" if item.get("key") == "cart" else ""}" href="{escape(str(item.get("href") or "#"))}" aria-label="{label}">{icon_html}</a>'
        )
...
def home_page(content: dict[str, Any] | None = None) -> str:
    ...
    news_image_markup = f'<img class="news-card__image" src="{escape(news_image_url)}" alt="{news_title}">' if news_image_url else '<div class="news-card__image news-card__image--fallback" aria-hidden="true"></div>'
    news_cta = f'<a class="news-card__cta" href="{escape(news_link_url)}">{news_link_label}</a>' if news_link_url and news_link_label else ""
...
{BASE_CSS}
    ...
    .home-subtitle {{ margin:0; font-size:clamp(42px,10vw,var(--home-subtitle-size)); line-height:.8; letter-spacing:.08em; color:var(--golden-malt); font-weight:var(--home-subtitle-weight); }}
    .home-news {{ min-height:100vh; display:grid; place-items:center; padding:96px min(6vw,72px); background:linear-gradient(180deg, var(--noble-hop), var(--deep-hop)); }}
    .news-card {{ width:min(960px,100%); display:grid; grid-template-columns:minmax(220px,340px) 1fr; gap:28px; align-items:center; background:var(--card-hop); border:1px solid rgba(199,177,102,.22); border-radius:30px; padding:28px; box-shadow:0 28px 70px rgba(0,0,0,.22); }}
    .news-card__image {{ width:100%; aspect-ratio:4/3; max-height:280px; object-fit:cover; border-radius:22px; background:rgba(246,241,227,.08); border:1px solid rgba(246,241,227,.12); }}
    ...
    .news-card__cta {{ display:inline-flex; margin-top:20px; padding:12px 18px; border-radius:999px; background:var(--golden-malt); color:var(--ink); text-decoration:none; font-weight:900; }}
    @media (max-width:760px) {{ .news-card {{ grid-template-columns:1fr; padding:20px; }} .home-logo {{ max-width:130px; max-height:130px; }} }}
  </style>
</head>
<body class="home-body">
{public_nav("home", site_content)}
  <main>
    <section class="home-hero" style="--home-title-size:{title_size}; --home-title-weight:{title_weight}; --home-subtitle-size:{subtitle_size}; --home-subtitle-weight:{subtitle_weight};">
