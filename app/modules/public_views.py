...
BASE_CSS = """
    ...
    * { box-sizing:border-box; }
    ...
    .nav-links { display:flex; align-items:center; justify-content:center; gap:16px; flex-wrap:wrap; font-size:14px; font-weight:700; }
...
def public_nav(active: str) -> str:
    ...
    links = "".join(f'<a class="{"is-active" if key == active else ""}" href="{href}">{label}</a>' for href, label, key in items)
    return f"""
  <nav class="top-nav" aria-label="Главная навигация">
    <a class="brand" href="/">Stamm Brewing</a>
    <div class="nav-links">{links}</div>
    <div class="nav-actions" aria-label="Соцсети и корзина">
      <a class="nav-icon" href="https://t.me/" aria-label="Telegram">TG</a>
      <a class="nav-icon" href="https://vk.com/" aria-label="VK">VK</a>
      <a class="nav-icon" href="https://instagram.com/" aria-label="Instagram">IG</a>
      <a class="nav-icon nav-icon--cart" href="/business#cart" aria-label="Корзина">🛒</a>
    </div>
  </nav>"""
...
<head>
  ...
  <style>
{BASE_CSS}
    .home-hero {{ min-height:58vh; display:grid; align-items:center; padding:72px min(6vw,72px); background:radial-gradient(circle at 78% 20%, rgba(199,177,102,.2), transparent 32%), linear-gradient(135deg, var(--noble-hop), var(--deep-hop)); }}
    .home-hero__inner {{ max-width:860px; }}
    .home-hero__eyebrow {{ color:var(--golden-malt); font-weight:900; letter-spacing:.14em; text-transform:uppercase; }}
    .home-hero h1 {{ margin:16px 0 0; font-size:clamp(42px,8vw,92px); line-height:.92; color:var(--foam); max-width:760px; }}
  ...
  <main>
    <section class="home-hero">
      <div class="home-hero__inner">
        <div class="home-hero__eyebrow">Welcome</div>
        <h1>Stamm Brewing — крафтовая пивоварня с уверенным характером.</h1>
      </div>
    </section>
    <section class="home-news" aria-label="Новости">
      <article class="news-card">
        ...
        <p>Каталог для партнёров собирается вокруг локальной модели товаров, синхронизации с МойСклад и аккуратного фирменного storefront.</p>
      </article>
    </section>
  </main>
</body>
...
def public_placeholder_page(title: str, active: str) -> str:
    return f"""<!doctype html>
<html lang="ru">
<head>
  {PUBLIC_HEAD}
