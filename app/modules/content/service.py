...
CONTACT_DEFAULTS = {
    "contacts_emails_json": '[{"label":"Основной","value":"info@stammbeer.ru"},{"label":"Заказы","value":"order@stammbeer.ru"}]',
    "contacts_phones_json": '[{"label":"Офис","value":"+7 (000) 000-00-00"},{"label":"Отдел продаж","value":"+7 (000) 000-00-01"}]',
...
def get_public_site_content(conn: sqlite3.Connection, include_hidden: bool = False) -> dict[str, Any]:
    ...
    for key in ("contacts_emails_json", "contacts_phones_json"):
        try:
            contacts[key.removeprefix("contacts_").removesuffix("_json")] = json.loads(str(contacts.get(key) or "[]"))
        except json.JSONDecodeError:
            contacts[key.removeprefix("contacts_").removesuffix("_json")] = []
    return {
...
def save_public_content(conn: sqlite3.Connection, data: dict[str, Any]) -> None:
    ...
    if any(key.startswith("contact_") or key.startswith("contacts_") for key in data):
        ...
        for index in range(8):
            ...
            if email_value:
                contact_emails.append({"label": email_label or "E-mail", "value": email_value})
            ...
            if phone_value:
                contact_phones.append({"label": phone_label or "Телефон", "value": phone_value})
        contact_values = {
