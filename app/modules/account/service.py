...
def refresh_customer_discount(conn: sqlite3.Connection, account: sqlite3.Row, force: bool = False) -> sqlite3.Row:
    if not force and not _discount_is_stale(account):
        return account
    try:
        counterparty = _build_moysklad_client(conn).fetch_counterparty(account["counterparty_href"])
...
def authenticate_customer(conn: sqlite3.Connection, email: str, password: str) -> sqlite3.Row | None:
    ...
    return refresh_customer_discount(conn, refreshed, force=True) if refreshed is not None else None
