...
def _save_counterparty_profile(
    conn: sqlite3.Connection, account_id: int, counterparty: dict[str, Any], previous_discount_percent: float | None = None
) -> sqlite3.Row | None:
...
def refresh_customer_discount(conn: sqlite3.Connection, account: sqlite3.Row, force: bool = False) -> sqlite3.Row:
    ...
    try:
        ...
        if counterparty is None:
            ...
            return account
        refreshed = _save_counterparty_profile(conn, account["id"], counterparty, float(account["discount_percent"] or 0))
