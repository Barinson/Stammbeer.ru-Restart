...
def content_management_page(user_email: str, content: dict[str, object], result: str | None = None, error: str | None = None) -> str:
    ...
    email_rows = "".join(
        f"""
        <div class='grid'>
          <label>Подпись e-mail #{index + 1}<input name='contact_email_label_{index}' value='{escape(str(item.get('label') or ''))}'></label>
          <label>E-mail #{index + 1}<input name='contact_email_value_{index}' type='email' value='{escape(str(item.get('value') or ''))}'></label>
        </div>
        """
    ...
    phone_rows = "".join(
        f"""
        <div class='grid'>
          <label>Подпись телефона #{index + 1}<input name='contact_phone_label_{index}' value='{escape(str(item.get('label') or ''))}'></label>
          <label>Телефон #{index + 1}<input name='contact_phone_value_{index}' value='{escape(str(item.get('value') or ''))}'></label>
        </div>
        """
    ...
    return page(
        ...
        <style>
          ...
          #cms-tab-home:checked ~ form .cms-panel-home, #cms-tab-contacts:checked ~ form .cms-panel-contacts, #cms-tab-business:checked ~ form .cms-panel-business, #cms-tab-nav:checked ~ form .cms-panel-nav {{ display:block; }}
        ...
        <form method="post" action="/admin/content/save" enctype="multipart/form-data">
          ...
          <section class="cms-tab-panel cms-panel-contacts">
            <div class="card">
              ...
              <h4>Телефоны</h4>
              {phone_rows}
              <label>Адрес завода</label>
              <input name="contacts_address" value="{escape(contacts_address)}">
              <label>Описание расположения</label>
              <textarea name="contacts_description" rows="4">{escape(contacts_description)}</textarea>
              <div class="grid">
                <label>Широта<input name="contacts_map_lat" value="{escape(contacts_map_lat)}"></label>
                <label>Долгота<input name="contacts_map_lng" value="{escape(contacts_map_lng)}"></label>
                <label>Zoom<input name="contacts_map_zoom" type="number" min="1" max="20" value="{escape(contacts_map_zoom)}"></label>
                <label>Подпись точки<input name="contacts_map_title" value="{escape(contacts_map_title)}"></label>
              </div>
              <div style="border-radius:18px; overflow:hidden; border:1px solid rgba(16,88,89,.16); margin-top:14px; max-width:520px;">
                <iframe title="Preview Яндекс.Карты" src="{map_preview_src}" width="100%" height="220" frameborder="0"></iframe>
              </div>
