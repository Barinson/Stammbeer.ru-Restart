...
def content_management_page(user_email: str, content: dict[str, object], result: str | None = None, error: str | None = None) -> str:
    ...
    logo_preview = f"<p><img src='{escape(logo_url)}' alt='Логотип главной' style='max-width:120px; max-height:120px; object-fit:contain;'></p>" if logo_url else "<p class='muted'>Логотип ещё не загружен. Будет показан фирменный знак-заглушка.</p>"
    menu_rows = "".join(
        f"""
        <tr>
    ...
    return page(
        ...
        <form method="post" action="/admin/content/save" enctype="multipart/form-data">
          <div class="card">
            ...
            <p class="muted">Управляет welcome-экраном главной: верхняя надпись, нижняя золотая надпись и logo asset над ними.</p>
            <label>Основная большая надпись</label>
            <input name="home_hero_title" value="{escape(str(home.get('home_hero_title') or 'STAMM'))}">
            <label>Подзаголовок под ней</label>
            <input name="home_hero_subtitle" value="{escape(str(home.get('home_hero_subtitle') or 'BREWING'))}">
            <div class="grid">
              <div>
                <label>Размер STAMM, px</label>
                <input name="home_hero_title_size_px" type="number" min="24" max="220" value="{escape(str(home.get('home_hero_title_size_px') or '152'))}">
              </div>
              <div>
                <label>Толщина STAMM</label>
                <input name="home_hero_title_weight" type="number" min="100" max="1000" step="50" value="{escape(str(home.get('home_hero_title_weight') or '950'))}">
              </div>
              <div>
                <label>Размер BREWING, px</label>
                <input name="home_hero_subtitle_size_px" type="number" min="24" max="220" value="{escape(str(home.get('home_hero_subtitle_size_px') or '112'))}">
              </div>
              <div>
                <label>Толщина BREWING</label>
                <input name="home_hero_subtitle_weight" type="number" min="100" max="1000" step="50" value="{escape(str(home.get('home_hero_subtitle_weight') or '950'))}">
              </div>
            </div>
            ...
            <p class="muted">При загрузке новый файл сохраняется локально и заменяет текущий logo asset. На главной и в preview он автоматически вписывается в безопасный размер без потери пропорций.</p>
          </div>
          <div class="card">
            <h3>Верхнее меню</h3>
