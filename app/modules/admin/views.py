...
def admin_catalog_page(user_email: str, items: list[dict[str, object]], beer_styles: list[dict[str, object]] | None = None, result: str | None = None, error: str | None = None) -> str:
    ...
    def container_selected(item: dict[str, object], value: str) -> str:
        selected = item.get("container_type_override") or item.get("container_type")
    ...
    if items:
        rows = "".join(
            ...
            <tr>
              ...
              <td>
                <form class="admin-catalog-style-form" method="post" action="/admin/catalog/container-assignment">
                  <input type="hidden" name="product_id" value="{escape(str(item.get('id')))}">
                  <select name="container_type">
                    <option value="keg" {container_selected(item, 'keg')}>Кег</option>
                    <option value="can" {container_selected(item, 'can')}>Банка</option>
                  </select>
                  <button type="submit">OK</button>
                </form>
                <small>Авто: {escape(str(item.get('container_type') or '—'))}</small>
              </td>
...
def content_management_page(user_email: str, content: dict[str, object], result: str | None = None, error: str | None = None) -> str:
    ...
    site_yandex_metrika_id = str(site.get("site_yandex_metrika_id") or "")
        ...
        <form method="post" action="/admin/content/save" enctype="multipart/form-data">
          ...
          <section class="cms-tab-panel cms-panel-site">
            <div class="card">
              ...
              <label>Yandex Metrika ID<input name="site_yandex_metrika_id" value="{escape(site_yandex_metrika_id)}" inputmode="numeric" placeholder="110732851"></label>
              <p class="muted">Если ID задан, счётчик Метрики подключается на всех публичных страницах через общий head. Если поле пустое — счётчик не выводится.</p>
              <label>Favicon</label>
