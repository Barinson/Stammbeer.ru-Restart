...
def content_management_page(user_email: str, content: dict[str, object], result: str | None = None, error: str | None = None) -> str:
    ...
    news_image_preview = f"<img class='cms-news-preview__image' src='{escape(news_image_url)}' alt='Изображение новости'>" if news_image_url else "<div class='cms-news-preview__image cms-news-preview__image--fallback' aria-hidden='true'></div>"
    menu_rows = "".join(
        f"""
        <tr>
    ...
    return page(
        ...
        <style>
          ...
          .cms-hero-preview__subtitle {{ margin:0; color:#C7B166; line-height:.8; letter-spacing:.08em; font-size:clamp(36px,7vw,{escape(subtitle_size)}px); font-weight:{escape(subtitle_weight)}; }}
          .cms-news-preview {{ display:grid; grid-template-columns:180px 1fr; gap:18px; align-items:center; border-radius:22px; padding:18px; background:#0d4b4c; color:#F6F1E3; }}
          .cms-news-preview__image {{ width:100%; aspect-ratio:4/3; border-radius:16px; object-fit:cover; background:rgba(246,241,227,.08); }}
          ...
          .cms-news-preview small {{ color:#C7B166; text-transform:uppercase; letter-spacing:.1em; font-weight:900; }}
          .cms-news-preview h4 {{ margin:8px 0; font-size:24px; }}
          .cms-news-preview p {{ margin:0; color:rgba(246,241,227,.78); }}
          .cms-news-preview a {{ display:inline-flex; margin-top:12px; padding:8px 12px; border-radius:999px; background:#C7B166; color:#172625; text-decoration:none; font-weight:900; }}
            ...
            <div class="cms-preview-grid">
              ...
              <article class="cms-news-preview" aria-label="Preview новости">
                ...
                <div>
                  ...
                  {f"<a href='{escape(news_link_url)}'>{escape(news_link_label)}</a>" if news_link_url and news_link_label else ""}
                </div>
              </article>
            </div>
          </div>
