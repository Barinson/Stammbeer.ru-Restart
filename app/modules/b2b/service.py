...
def build_customer_order_payload(
    ...
    if comment.strip():
        payload["description"] = comment.strip()
    return payload
...
def send_order_to_moysklad(
    ...
    payload = build_customer_order_payload(order_number, customer, order_items, store, organization, comment)
    try:
    ...
    return {
        ...
        "externalOrderName": response.get("name"),
        "store": store,
