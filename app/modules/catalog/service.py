...
def _price_payload(price_minor: int | None, currency: str | None, discount_percent: float = 0.0) -> dict[str, Any]:
    if price_minor is None:
        return {"visibility": "hidden", "label": "Цена по запросу"}
    normalized_discount = max(0.0, min(float(discount_percent or 0), 100.0))
    effective_minor = round(price_minor * (100 - normalized_discount) / 100)
    return {
        "visibility": "public",
        "amountMinor": effective_minor,
        "baseAmountMinor": price_minor,
        "currency": currency or "RUB",
        "label": f"{effective_minor / 100:,.0f} ₽".replace(",", " "),
        "baseLabel": f"{price_minor / 100:,.0f} ₽".replace(",", " "),
        "discountPercent": normalized_discount,
        "isPersonalized": normalized_discount > 0,
    }


def public_catalog(conn: sqlite3.Connection, container_type: str | None = None, customer_discount_percent: float = 0.0) -> dict[str, Any]:
        ...
        items.append(
            {
                ...
                "price": _price_payload(row["price_minor"], row["currency"], customer_discount_percent),
                "availability": {
                    "status": availability,
    ...
    return {
        ...
        "meta": {
            ...
            "pricingMode": "personal" if customer_discount_percent else "base",
