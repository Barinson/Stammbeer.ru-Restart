...
def extract_description(row: dict[str, Any]) -> str | None:
    value = row.get("description") or row.get("comment") or row.get("notes")
    if value is None:
        return None
...
def extract_alcohol_percent(description: str | None) -> float | None:
    if not description:
        return None
    text = str(description).replace(",", ".")
    keyword_patterns = (
        r"(?:abv|alc\.?|alcohol|алк\.?|алкоголь|крепость)[^0-9]{0,24}(\d{1,2}(?:\.\d{1,2})?)\s*(?:%|об|град)?",
        r"(\d{1,2}(?:\.\d{1,2})?)\s*(?:%|об\.?\s*%|град)",
    )
...
def extract_sale_prices(row: dict[str, Any]) -> list[dict[str, Any]]:
    prices: list[dict[str, Any]] = []
    for price in row.get("salePrices") or []:
        price_type = price.get("priceType") or {}
        reference = normalize_api_reference(price_type) if isinstance(price_type, dict) and price_type else {
            "id": None,
            "href": None,
            "name": price.get("priceTypeName"),
            "meta": {},
        }
        prices.append(
            {
                "value": money_to_minor(price.get("value")),
                "currency": price.get("currency") or "RUB",
                "priceTypeId": reference.get("id"),
                "priceTypeHref": reference.get("href"),
                "priceTypeName": reference.get("name"),
                "priceTypeMeta": reference.get("meta") or {},
            }
        )
    return prices


def sale_price_type_names(prices: list[dict[str, Any]]) -> list[str]:
    names: list[str] = []
    for price in prices:
        if price.get("value") is None:
            continue
        name = price.get("priceTypeName") or price.get("priceTypeId") or price.get("priceTypeHref") or ""
        names.append(str(name).strip())
    return names


def row_with_sale_prices(client: MoyskladClient, row: dict[str, Any], reference: dict[str, Any], stats: dict[str, Any]) -> dict[str, Any]:
    has_sale_prices = bool(row.get("salePrices"))
    has_description = bool(extract_description(row))
    if has_sale_prices:
        stats["salePricesFromAssortmentRows"] = stats.get("salePricesFromAssortmentRows", 0) + 1
    if has_sale_prices and has_description:
        return row
    try:
        detailed = client.fetch_assortment_entity(reference["href"])
    except Exception as exc:
        stats.setdefault("salePriceFetchErrors", []).append({"href": reference["href"], "error": str(exc)})
        return row
    merged = dict(row)
    merged.update({key: value for key, value in detailed.items() if value is not None})
    if not has_sale_prices and detailed.get("salePrices"):
        stats["salePricesFetchedByHref"] = stats.get("salePricesFetchedByHref", 0) + 1
    if not has_description and extract_description(detailed):
        stats["descriptionsFetchedByHref"] = stats.get("descriptionsFetchedByHref", 0) + 1
    if not merged.get("salePrices"):
        stats["salePricesMissingAfterFetch"] = stats.get("salePricesMissingAfterFetch", 0) + 1
    return merged

def extract_sale_price_minor(row: dict[str, Any]) -> int | None:
    """Return MoySklad "Цена продажи" for one SKU in minor money units."""
    for price in extract_sale_prices(row):
        price_type_name = str(price.get("priceTypeName") or "").strip().lower()
...
def db_write_debug(row: dict[str, Any], reference: dict[str, Any], snapshot: dict[str, float], action: str, product_id: int | None, reason: str | None = None) -> dict[str, Any]:
    available = snapshot["availableQuantity"]
    sale_prices = extract_sale_prices(row)
    return {
        "name": row.get("name") or reference["name"],
        "id": reference["id"],
        "href": reference["href"],
        ...
        "rawSalePricesCount": len(row.get("salePrices") or []),
        "savedPriceTypes": sale_price_type_names(sale_prices),
    }
...
def run_manual_catalog_sync(conn: sqlite3.Connection, user_id: int | None = None, diagnostic_mode: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        stats["salePricesFromAssortmentRows"] = 0
        stats["salePricesFetchedByHref"] = 0
        stats["salePricesMissingAfterFetch"] = 0
        stats["salePriceFetchErrors"] = []
        ...
        for row, snapshot in positive_rows:
            ...
            priced_row = row_with_sale_prices(client, row, reference, stats)
            price_minor = extract_sale_price_minor(priced_row)
            sale_prices = extract_sale_prices(priced_row)
            ...
            if existing:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        row.get("externalCode"), container_type, price_minor, price_type_prices_json, available_quantity, image_url, description, alcohol_percent, availability_for_stock(available_quantity),
                ...
                cursor = conn.execute(
                    ...
                    INSERT INTO products (
                        ...
                        container_type, price_minor, currency, price_type_prices_json, stock_quantity, image_url, description, alcohol_percent, availability_status,
            ...
            conn.execute(
                ...
                SET price_minor = ?, price_type_prices_json = ?, currency = 'RUB',
                    image_url = coalesce(?, image_url), alcohol_percent = coalesce(?, alcohol_percent), last_catalog_sync_at = ?
                WHERE product_id = ?
                """,
                (price_minor, price_type_prices_json, image_url, alcohol_percent, now, product_id),
            )
            if diagnostic_mode and len(diagnostic_payload["dbWrites"]) < 10:
                entry = db_write_debug(priced_row, reference, snapshot, "update" if existing else "create", product_id)
                entry["savedImageUrl"] = image_url
                entry["savedAlcoholPercent"] = alcohol_percent
