...
def folder_matches(row: dict[str, Any], source_folder_href: str, include_child_folders: bool) -> bool:
    folder_href = (((row.get("productFolder") or {}).get("meta") or {}).get("href"))
    if not folder_href:
        return False
    if folder_href == source_folder_href:
        return True
    return include_child_folders and folder_href.startswith(source_folder_href.rstrip("/") + "/")
...
def run_manual_catalog_sync(conn: sqlite3.Connection, user_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        scoped_rows = [
            row for row in assortment_rows
            if folder_matches(row, settings["source_product_folder_href"], bool(settings["include_child_folders"]))
        ]
        ...
        if seen_hrefs:
            ...
            params = [settings["source_product_folder_href"], *seen_hrefs]
            out_rows = conn.execute(
                f"""
                SELECT id FROM products
                WHERE source_folder_href = ? AND external_href NOT IN ({placeholders}) AND sync_state = 'active'
                """,
                params,
            ).fetchall()
            ...
            conn.execute(
                ...
                WHERE source_folder_href = ? AND external_href NOT IN ({placeholders})
                """,
                [now, *params],
            )
