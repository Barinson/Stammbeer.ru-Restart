...
def stock_snapshots_by_key(stock_rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    snapshots: dict[str, dict[str, float]] = {}
    for row in stock_rows:
        snapshot = stock_snapshot(row)
...
def run_manual_catalog_sync(conn: sqlite3.Connection, user_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        stock_by_key = stock_snapshots_by_key(stock_rows)
        ...
        for row in scoped_rows:
            ...
            href = reference["href"]
            snapshot = stock_by_key.get(href) or stock_by_key.get(reference["id"], stock_snapshot(None))
