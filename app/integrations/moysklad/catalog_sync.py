...
def stock_snapshot(row: dict[str, Any] | None) -> dict[str, float]:
    ...
    if "available" in row:
        available = number_value(row.get("available"))
    elif "quantity" in row:
        available = max(number_value(row.get("quantity")) - reserve, 0.0)
    else:
        # In MoySklad stock report rows, `stock` is the positive free stock value used for availability.
        available = stock
    return {
        "stock": stock,
        "reserve": reserve,
...
def stock_snapshots_by_href(stock_rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    snapshots: dict[str, dict[str, float]] = {}
    for row in stock_rows:
        assortment_href = ((row.get("assortment") or {}).get("meta") or {}).get("href")
        if assortment_href:
            snapshots[assortment_href] = stock_snapshot(row)
    return snapshots
...
def run_manual_catalog_sync(conn: sqlite3.Connection, user_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        stock_by_href = stock_snapshots_by_href(stock_rows)
        ...
        for row in scoped_rows:
            ...
            snapshot = stock_by_href.get(href, stock_snapshot(None))
            ...
            if existing:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        row.get("externalCode"), container_type, price_minor, available_quantity, availability_for_stock(available_quantity),
