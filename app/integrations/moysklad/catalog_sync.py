...
def stock_snapshot_for_reference(reference: dict[str, Any], stock_by_key: dict[str, dict[str, float]]) -> tuple[dict[str, float], str | None]:
    for key in (reference["href"], reference["id"]):
...
def run_manual_catalog_sync(conn: sqlite3.Connection, user_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if len(scoped_rows) > len(diagnostic_sample):
            stats["stockMatched"] = sum(1 for row in scoped_rows if any(key in stock_by_key for key in (normalize_api_reference(row)["href"], normalize_api_reference(row)["id"])))
