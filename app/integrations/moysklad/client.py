...
def extract_personal_discount_percent(row: dict[str, Any]) -> float:
    for entry in row.get("discounts") or []:
        candidates = [entry.get("personalDiscount")]
        if isinstance(entry.get("discount"), dict):
            candidates.append(entry["discount"].get("personalDiscount"))
...
class MoyskladClient:
    ...
    def find_counterparty_by_inn(self, inn: str) -> dict[str, Any] | None:
        ...
        rows = self._fetch_collection_rows(
            "entity/counterparty",
            limit=100,
            query={"filter": f"inn={normalized_inn}"},
        )
        for row in rows:
    ...
    def fetch_counterparty(self, counterparty_href: str) -> dict[str, Any]:
        ...
        return normalize_counterparty(self._request(path))
